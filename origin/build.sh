...
./isorespin.sh -i "$INPUTFOLDER""$ISOFILE" \
	...
	-p "$installpackages" \
	-f /docker-input/wrapper-network.sh \
	-f /docker-input/wrapper-nvidia.sh \
	-f /docker-input/services/gpuoff.service \
	-c /docker-input/wrapper-network.sh \
	-c /docker-input/wrapper-nvidia.sh \
	-g "" \
	-g "$GRUBOPTIONS"
